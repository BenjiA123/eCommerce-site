...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user)
    return next(new AppError("This email is not recorded email address ", 404));
...
exports.createUser = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    // Deletes users that an email could't be sent to
    user.deleteOne();
    return next(
      new AppError(
        "Their was an error sending this email. Pleaes retry again later",
        500
      )
    );
  }
